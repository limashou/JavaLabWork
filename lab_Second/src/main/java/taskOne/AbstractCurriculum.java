...
public abstract class AbstractCurriculum  {
    ...
    public String findWord(Lecture[] lectures, String word) {
        ...
        for (Lecture lecture : lectures) {
            if (lecture.containsSpecificWord(word)) {
                find = lecture.getTopic();
            }
        }
        return find;
    }
    ...
    public void printLecture(Lecture[] lectures) {
        for (int i = 0; i < lectures.length; i++) {
            System.out.println( '\"'+ lectures[i].getData() +'\"' + "Number of students: " + lectures[i].getStudents() + "\t" + '\"' + lectures[i].getTopic() + '\"');
        }
    }
    ...
    public void test() {
        ...
        System.out.println(findWord(getLecture(), word));
