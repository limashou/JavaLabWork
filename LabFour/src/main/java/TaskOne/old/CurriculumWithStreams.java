...
public class CurriculumWithStreams extends CurriculumWithArrayList {
    ...
    public CurriculumWithStreams() {super();
        setLectures(new ArrayList<Lecture>());
        setName("");
        setLectureSurname("");
    }
    /**
    ...
    public String findLowestAmountOfStudent() {
       ...
       return Arrays.stream(lectures).filter(lecture -> lecture.getStudents() == minStudents).findFirst().orElse(null).getTopic();
    }
    /**
    ...
    public Lecture[] sortByCommentLength() {
        Lecture[] lectures = getLecture();
        setLectures(Arrays.stream(lectures).sorted(Comparator.comparingInt(Lecture::calculateWordsCount)).collect(Collectors.toList()));
        return getLecture();
    }
    /**
     ...
     * @return
     */
    public Lecture[] sortByAlph() {
        Lecture[] lectures = getLecture();
        setLectures(Arrays.stream(lectures).sorted(Comparator.comparing(Lecture::getTopic)).collect(Collectors.toList()));
        return getLecture();
    }
    /**
    ...
    public void test() {
        ...
        System.out.println("=====================\nSorting:");
        System.out.println("=====================\nSorting by alphabet:");
        System.out.println(Arrays.stream(sortByAlph())
                .map(Lecture::toString)
                .collect(Collectors.joining("\n")));
...
//    public void add(String date,String topic,int students){
//        Lecture[] lectures = getLecture();
//        lectures[].setData(date);
//        lectures[].setTopic(topic);
//        lectures[].setStudents(students);
    //}
    /**
    ...
    public void xStreamSerialXML(String file) {
        ...
        xStream.alias("CurriculumWithStreams", CurriculumWithStreams.class);
        xStream.alias("Lecture", Lecture.class);
    ...
    public void xStreamDeserialXML(String file) {
        ...
        setLectures(curriculumWithStreams.getLectures());
    ...
    public static void main(String[] args) {
        ...
        List<Lecture> lectures = new ArrayList<>();
        lectures.add(new Lecture("First step in Java." , 24,"09.04"));
        lectures.add(new Lecture("Work with files.", 19,"09.28"));
        lectures.add(new Lecture("Arrays and list." , 20,"11.03"));
        lectures.add(new Lecture("Simplex method." ,17,"09.23"));
